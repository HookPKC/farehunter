...
資料來源為 **Amadeus Self-Service API**（合法官方 API，非爬蟲）。

## 架構

```
config.yaml ──> runner ──> Amadeus API ──> parse ──> SQLite (prices.db)
                                                        │
                                              analyzer（三條規則）
                                                        │
...
- `src/farehunter/amadeus.py` — OAuth2 + Flight Offers Search，含 401/429 重試
...
2. My Self-Service Workspace → Create New App → 取得 API Key / Secret
3. 設定 `AMADEUS_CLIENT_ID`、`AMADEUS_CLIENT_SECRET`
4. `AMADEUS_ENV=test` 先用測試環境驗證流程；**測試環境是快取資料，價格不是即時的**，
   流程跑通後到 Workspace 申請 production key（免費、有每月免費額度），改
   `AMADEUS_ENV=production` 才會拿到真實即時價格
...
2. Repo → Settings → Secrets and variables → Actions，新增 secrets：
   `AMADEUS_CLIENT_ID`、`AMADEUS_CLIENT_SECRET`，以及選用的
   `TELEGRAM_BOT_TOKEN`、`TELEGRAM_CHAT_ID`、`LINE_CHANNEL_ACCESS_TOKEN`、`LINE_USER_ID`
3. （切換正式環境時）Variables 新增 `AMADEUS_ENV=production`
4. Actions 頁面手動觸發一次 `FareHunter Monitor` 確認能跑
5. 之後每 6 小時自動執行，`prices.db` 會自動 commit 回 repo 累積歷史價格
...
- **LCC 覆蓋不完整**：Amadeus Self-Service 對廉航（台灣虎航、樂桃等）覆蓋有限，
  拿到的主要是華航／長榮等全服務航空的公告票價，看不到促銷會員價
- **統計規則需要時間**：`new_low` / `big_drop` 要累積約 2–4 週資料才會啟用，
  這是刻意設計，避免上線第一天的誤報
- **test 環境價格非即時**：驗證流程用，正式監控請切 production
