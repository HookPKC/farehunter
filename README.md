...
| `new_low` | 價格 < **整條航線**的歷史最低 | 航線累積 ≥ 30 筆觀察 |
| `big_drop` | 價格 ≤ **該出發日**中位數 × 75% | 該出發日累積 ≥ 30 筆觀察 |

兩條統計規則的比較基準刻意不同，因為它們問的問題不同：

- **`new_low` 比整條航線** —— 「這條航線史上最便宜」是極值事件，罕見而明確。
  改用單日基準會退化成「這天又刷新自己的紀錄」，那在累積初期天天發生
  （實測 5 週從 3 次暴增到 147 次）。
- **`big_drop` 比該出發日** —— 「這天相對它自己平常的價格反常地便宜」必須用
  單日基準。同一條航線不同出發日的均價可差到數倍（實測 TPE→KIX 從 5,853 到
  45,862），用混算的中位數會把「這天本來就貴」誤判成「大跌」。

### 去重

同一個行程（航線＋去回程日＋航空公司＋價格狀態）在 **30 天內**只通知一次，
除非新價格比**已通知過的最低價**再便宜 10% 以上，或狀態升級為「已驗證」。

`unverified` 與 `conflict` 視為同一個狀態桶——兩者的差別只在於當下有沒有一筆
落在 48 小時參考窗內的 Google 觀測，價格本身沒有變，不構成新消息。
...
AI 接手／交接時的通用開場指令（適用任何模型）見 [`docs/HANDOFF_AI.md`](docs/HANDOFF_AI.md)。
